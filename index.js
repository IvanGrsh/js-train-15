...
function findMinMax(numbers) {
  if (typeof Array.from(numbers) !== true) {
...
function sumPositiveNumbers(numbers) {
  if (typeof Array.isArray(numbers) !== true) {
    let sum = 0;
...
function getFractionalPart(num) {
  if (typeof num === "number") {
    let par = num - Math.trunc(num);
    return Math.fround(par);
  } else {
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  if (Array.isArray(products) === true) {
    if (typeof maxPrice === "number") {
      let totalPrice = products.reduce(({ price }, acum) => acum + price, 0);
